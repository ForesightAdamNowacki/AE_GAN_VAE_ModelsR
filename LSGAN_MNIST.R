# ------------------------------------------------------------------------------
# LEAST SQUARES GENERATIVE ADVERSARIAL MODEL
# ------------------------------------------------------------------------------
# Data:
...
# ------------------------------------------------------------------------------
# Generator:
build_generator <- function(inputs, image_size, activation = 'sigmoid'){
  ...
  # * activation (String): Name of output activation function
...
test_build_discriminator <- build_discriminator(inputs = keras::layer_input(shape = base::c(28, 28, 1)),
                                                activation = NULL); test_build_discriminator
...
build_and_train_models <- function(){
  ...
  discriminator <- build_discriminator(inputs = inputs, activation = NULL)
  discriminator %>% keras::compile(loss = "mean_squared_error", 
                                   optimizer = optimizer,
...
numbers <- 100
plot_final_images(generator = keras::load_model_hdf5("lsgan/lsgan_generator.hdf5", compile = FALSE),
                  noise_input = base::matrix(data = stats::runif(n = numbers * 100, min = -1, max = 1), nrow = numbers, ncol = 100))

# ------------------------------------------------------------------------------
# Training visualization:
base::list.files(path = base::paste(base::getwd(), "lsgan", sep = "/"), pattern = "*000.png", full.names = TRUE) %>%
  stringr::str_sort(numeric = TRUE) %>%
  magick::image_read() %>%
  magick::image_join() %>%
  magick::image_animate(delay = 0.5, loop = 1) %>%
  magick::image_write_video(base::paste("lsgan", "lsgan_training.gif", sep = "/"))

# ------------------------------------------------------------------------------
# Accuracy and Loss visualization:
text_size <- 7
readr::read_csv("lsgan/lsgan.csv") %>%
  ...
  ggplot2::labs(x = "Iteration", y = "Value", title = "LSGAN Model") +
  ggplot2::theme(plot.title = element_text(size = text_size, color = "black", face = "bold", hjust = 0.5, vjust = 0.5),
                 ...
                 strip.text = element_text(size = text_size, face = "bold")) -> lsgan_results_plot; lsgan_results_plot
ggplot2::ggsave(base::paste("lsgan", "lsgan_results.png", sep = "/"), plot = lsgan_results_plot, units = "cm", width = 25, height = 25)
